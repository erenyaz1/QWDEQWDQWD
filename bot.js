...
client.on("guildMemberAdd", member => {
  member.roles.add("803271042780758067"); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
\`˃\` Sunucumuza hoş geldin <@` +
      ...
      `>, hesabın \`` +
      gecen +
      `\` tarihinde oluşturulmuş ve ` +
      kontrol +
      `

     \`˃\` Kayıt olmak için \`Confirmation\` odalarına geçip <@&803271042831220772> yetkilisine teyit vererek kayıt olabilirsin.

          \`˃\` Sunucumuz \`♢\` tagını kullanıcı adına ekleyerek bizelerden birisi olabilirsin.

     \`˃\` Seninle beraber sunucumuz toplam ` +
      üyesayısı +
      ` kişiye ulaştı bizi şereflendirdin ! 

\`˃\` İçerize keyifli vakitler geçirmeni diler, sunucumuzun <#803271043342925829> kısmına göz atmanı tavsiye ederiz.**`
  );
});
...
client.on("guildMemberAdd", member => {
  ...
  if (!x.includes("önce") || x.includes("sonra") || x == " ") {
    const kytsz = member.guild.roles.cache.find(
      r => r.id === "803271042780758067"
    );
    var rol = member.guild.roles.cache.get("803271042813394984"); // ŞÜPHELİ HESAP ROLÜNÜN İDSİNİ GİRİN
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get("803271042663448576"); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var ekipTag = "♢"; // Buraya Ekip Tag
  var ekipRolü = "803271042780758072"; // Buraya Ekip Rolünün ID
  var logKanali = "803271043027304464"; // Loglanacağı Kanalın ID
...
client.on("guildMemberAdd", member => {
  let sunucuid = "803271042663448576"; //Buraya sunucunuzun IDsini yazın
  let tag = "♢"; //Buraya tagınızı yazın
  let rol = "803271042780758072"; //Buraya tag alındığı zaman verilecek rolün IDsini yazın
  let channel = client.guilds.cache
    .get(sunucuid)
    .channels.cache.find(x => x.name == "803271043027304464"); //tagrol-log yerine kendi log kanalınızın ismini yazabilirsiniz
  if (member.user.username.includes(tag)) {
    ...
    const tagalma = new Discord.MessageEmbed()
      ...
      .setTimestamp();
    client.channels.cache.get("803271043027304464").send(tagalma);
  }
});
